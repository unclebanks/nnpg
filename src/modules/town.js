...
import { $, getPokemonDataByName } from "./utilities";
...
export class Town {
    ...
    buyPokeCoinItem(index,player,dom) {
    ...
    buyBattleCoinItem(index,player,dom) {
    ...
    buyCatchCoinItem(index,player,dom) {
    ...
    renderTrader(player) {
        this.renderBuyTrader(player);
    };
    calculatePokeValue(poke, demandMult = 1) {
        const shinyMult = (poke.shiny()) ? 1500 : 1;
        return Math.floor((poke.level() / 4) * shinyMult * demandMult);
    };
    buyPoke(index,player,dom) {
        const pokeValue = 100000;
        if (player.currencyAmount.pokecoins < pokeValue) {
            return false;
        } else {
            const poke = getPokemonDataByName(this.traderPoke[index]);
            const newPoke = new Pokemon(poke, 30, false, Math.random() < (1 / (1 << 5 << 8)));
